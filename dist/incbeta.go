...
//
// Derived from gonum.org/v1/gonum/mathext/internal/cephes
//
// Original Cephes Math Library, Release 2.3: March, 1995
// Copyright 1984, 1995 by Stephen L. Moshier
// Made freely available without support or guarantee.
...
// regIncBeta returns the regularized incomplete beta function I(x;a,b).
// It is defined as:
//
//	I(x;a,b) = B(x;a,b) / B(a,b)
//	         = Γ(a+b) / (Γ(a)*Γ(b)) * ∫₀ˣ u^(a-1) * (1-u)^(b-1) du
func regIncBeta(aa, bb, xx float64) float64 {
	if aa <= 0 || bb <= 0 {
		panic("cephes: parameter out of bounds")
	}
	if xx <= 0 || xx >= 1 {
		if xx == 0 {
			return 0
...
// Lbeta is defined as: Ln(Γ(a)Γ(b)/Γ(a+b))
